...
//
// Симуляция (Params/Stats/Simulate) живёт в internal/rotation — вынесена
// туда, чтобы cmd/regime мог переиспользовать её как "ranging"-плечо
// regime-switching аллокатора вместо повторной реализации той же логики.
package main
...
func run(cacheDir string, totalDays, holdoutDays, folds, topN int) error {
	...
	closes := make(map[string][]rotation.DailyBar, len(symbols))
	var usable []string
	...
	var results []scored
	for _, p := range grid {
		st := rotation.Simulate(closes, symbols, p.LookbackDays, splitIdx, p, startEquity, feeRate, slip)
	...
	for _, r := range top {
		...
		for _, fb := range foldBounds {
			st := rotation.Simulate(closes, symbols, fb.start, fb.end, r.p, startEquity, feeRate, slip)
...
func buildGrid() []rotation.Params {
	lookbacks := []int{3, 5, 7, 10, 14, 20}
	topKs := []int{1, 2, 3, 4, 5}
	rebalances := []int{1, 3, 7}

	var grid []rotation.Params
	for _, lb := range lookbacks {
		for _, k := range topKs {
			for _, rb := range rebalances {
				grid = append(grid, rotation.Params{LookbackDays: lb, TopK: k, RebalanceEvery: rb})
			}
		}
	}
	return grid
...
func loadOrFetchDaily(ctx context.Context, client *futures.Client, cacheDir, symbol string, start, end time.Time, totalDays int) ([]rotation.DailyBar, error) {
...
func toDailyBars(candles []domain.Candle) []rotation.DailyBar {
	bars := make([]rotation.DailyBar, len(candles))
